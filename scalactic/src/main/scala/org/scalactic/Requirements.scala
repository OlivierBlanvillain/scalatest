...
trait Requirements {
   ...
   */
  def require(condition: Boolean)(implicit prettifier: Prettifier): Unit =
    if (!condition) throw new IllegalStateException("MM")
    // macro RequirementsMacro.require
   ...
   */
  def require(condition: Boolean, clue: Any)(implicit prettifier: Prettifier): Unit =
    if (!condition) throw new IllegalStateException(clue.toString + " MM")
    // macro RequirementsMacro.requireWithClue
   ...
   */
  def requireState(condition: Boolean)(implicit prettifier: Prettifier): Unit =
    if (!condition) throw new IllegalStateException("MM")
    // macro RequirementsMacro.requireState
   ...
   * @throws NullPointerException if <code>message</code> is <code>null</code>.
   */
  def requireState(condition: Boolean, clue: Any)(implicit prettifier: Prettifier): Unit =
    if (!condition) throw new IllegalStateException(clue.toString + " MM")
    // macro RequirementsMacro.requireStateWithClue
   ...
   */
  def requireNonNull(arguments: Any*)(implicit prettifier: Prettifier, pos: source.Position): Unit =
    if (!arguments.forall(null.!=)) throw new NullArgumentException(arguments.toString + " MM")
    // macro RequirementsMacro.requireNonNull
}
