...
final class ResultOfNotWordForAny[T](val left: T, val shouldBeTrue: Boolean, val prettifier: Prettifier, val pos: source.Position) {
   ...
   * </pre>
   **/
  def be(aType: ResultOfATypeInvocation[_]): Assertion = macro TypeMatcherMacro.assertATypeShouldBeTrueImpl // MM
   ...
   * </pre>
   **/
  def be(anType: ResultOfAnTypeInvocation[_]): Assertion = macro TypeMatcherMacro.assertAnTypeShouldBeTrueImpl // MM
   ...
   * </pre>
   **/
  def matchPattern(right: PartialFunction[Any, _]) = macro MatchPatternMacro.matchPattern // MM
