...
import sbt.testing.{SubclassFingerprint, AnnotatedFingerprint, TaskDef, EventHandler, Selector, Logger, Task, Fingerprint, EventHandler, SuiteSelector, SuiteSelector, TestSelector, TestWildcardSelector, NestedSuiteSelector, NestedTestSelector, TaskDef, Selector, Task, EventHandler, Logger, Logger, Selector, TaskDef, TaskDef, TaskDef, TaskDef, TaskDef, TaskDef, Task, Runner, Status, OptionalThrowable}
...
class Framework extends sbt.testing.Framework {
  ...
  private def runSuite(
    ...
    try {
      ...
      // statusList.put(status) TODO OLIVIER
    ...
    def done = {
      if (!isDone.getAndSet(true)) {
        ...
        statusList.asScala.foreach { s =>
          try {
            // s.waitUntilCompleted() // TODO: OLIVIER
          }
          catch {
